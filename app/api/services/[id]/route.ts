...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  return authProtectedEndpoint(request, async () => {
    try {
      ...
      await db.query(
        'UPDATE services SET title = ?, description = ?, icon = ?, image_url = ?, `order` = ?, is_published = ? WHERE id = ?',
        [title, description, icon || '', image_url || '', order || 0, is_published || false, params.id]
      );
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  return authProtectedEndpoint(request, async () => {
    try {
      await db.query('DELETE FROM services WHERE id = ?', [params.id]);
      return NextResponse.json({ success: true });
    } catch (error) {
      const { message, statusCode } = handleError(error);
      return NextResponse.json({ error: message }, { status: statusCode });
